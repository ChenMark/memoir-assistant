/**
 * 忆往昔 SDK — AI 对话 & 回忆录引导
 * 10 维度 × 10 提示词的引导式访谈系统
 */

import type { ChatMessage, InterviewDimension, InterviewState } from '../types/index.js'
...
function generateSmartMockReply(
  ...
  // 用户提问
  if (
    userLower.includes('你知道') ||
    userLower.includes('你了解') ||
    userLower.includes('你清楚')
  ) {
  ...
  // 用户表示话题才刚开始
  if (
    userLower.includes('才刚刚开始') ||
    userLower.includes('还没说完') ||
    userLower.includes('继续')
  ) {
  ...
  // 用户说换话题
  if (
    userLower.includes('下一个') ||
    userLower.includes('换话题') ||
    userLower.includes('好了')
  ) {
    const currentIdx = INTERVIEW_DIMENSIONS.findIndex((d) => d.id === dimensionId)
    const nextDim = INTERVIEW_DIMENSIONS[currentIdx + 1]
    if (nextDim) {
...
/**
 * 发送聊天消息，获取 AI 响应
 * 支持自定义 AIProvider，无 provider 时使用智能 Mock
 */
export async function chat(
  ...
  if (provider) {
    try {
      const systemPrompt = SYSTEM_PROMPT.replace('{dimensionName}', dimensionName).replace(
        '{dimensionDesc}',
        dimensionDesc
      )
...
export async function generateStory(
  ...
  if (provider) {
    try {
      ...
      const prompt = `以下是用户的访谈记录，请根据这些内容撰写回忆录：\n\n${userContent}`
      return await provider.generateText(prompt, STORY_SYSTEM_PROMPT)
    } catch {
      // AI 失败，降级到 Mock
    }
  }
...
function generateMockStory(messages: ChatMessage[]): string {
  ...
  messages.forEach((msg) => {
    if (msg.role === 'user') {
      ...
      else if (msg.content.includes('亲人') || msg.content.includes('父母'))
        currentDimension = '亲人故事'
      else if (msg.content.includes('人生') || msg.content.includes('感慨'))
        currentDimension = '人生感悟'
    }
  })
 ...
 * 根据维度 ID 获取下一个维度
 */
export function getNextDimension(currentId: string): InterviewDimension | null {
  const idx = INTERVIEW_DIMENSIONS.findIndex((d) => d.id === currentId)
  return idx >= 0 && idx < INTERVIEW_DIMENSIONS.length - 1
    ? INTERVIEW_DIMENSIONS[idx + 1]
    : null
}
 ...
 * 获取系统提示词模板
 */
export function getSystemPrompt(dimensionName: string, dimensionDesc: string): string {
  return SYSTEM_PROMPT.replace('{dimensionName}', dimensionName).replace(
    '{dimensionDesc}',
    dimensionDesc
  )
}
